...
<template>
  <UICard
    ...
    :defaults="{ base: ' border-l-2 border-emerald-400 overflow-hidden' }"
  >
    <template #header>
    ...
    <template #default>
      <div class="divide-y divide-gray-200 dark:divide-gray-700">
        ...
        <div class="flex flex-col p-3 gap-2">
          ...
          <div
            class="grid grid-cols-12 text-sm items-center"
            v-for="item in data.examDetails"
            :key="item.exam_id"
          >
            ...
            <div class="col-span-1">
              <span class="text-red-500">{{ item.correctAnswers }}</span
              >/{{ item.totalQuestions }}
            </div>
          </div>
        </div>
        <div class="flex items-center py-5 px-2">
          <h1 class="text-3xl font-bold">
            Score: <span class="text-red-500">{{ correctAnswer }}</span
            >/{{ totalQuestion }}
          </h1>
        </div>
      </div>
