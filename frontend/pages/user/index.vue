...
const { summaryData, courseData, summaryScores, isFinished, examAttempts } =
...
const averageSuccessRate = computed(() => {
  ...
  const totalSuccessRate = exams.reduce(
    (acc, exam) => acc + exam.success_rate,
    0
  );
...
const firstMiddleName = computed(
  () =>
    `${data.value?.summaryData.first_name} ${data.value?.summaryData.middle_name
...
const { percentage, colors, detail, hexColor, description } =
  usePercentage(summaryScores);
const items = [
  '/news/1.jpg',
  '/news/2.jpg',
  '/news/5.jpg',
...
<template>
  <Suspense>
        ...
        <div class="col-span-12 lg:col-span-8 md:col-span-6 flex flex-col gap-3">
          <div
            ...
            <div class="flex flex-col gap-4 pr-12 flex-1">
              <div class="flex items-center gap-5">
                ...
                <div class="flex flex-col">
                  <h1 class="text-white font-semibold text-2xl lg:text-2xl">
                    Hello,
                    <span class="uppercase">{{ data?.summaryData.last_name || '' }}
                    </span>
                    !
                  </h1>
              ...
              <UProgress :value="summaryScores.correctAnswers" :max="summaryScores.totalQuestions" color="green">
              </UProgress>
              <div v-if="isFinished" class="text-center font-semibold pb-2 uppercase text-white/90">
                You already taken the exam!
              </div>
              <UButton v-else :to="{ name: 'user-information' }" :ui="{
                base: 'text-center',
                inline: 'inline-block',
                variant: {
                  solid: 'dark:bg-white',
                },
              }" class="py-2 px-4" color="gray" variant="solid">TAKE THE EXAM</UButton>
            </div>
            <NuxtImg src="/bg-image/herobg.png" fit="cover" height="180" class="mb-4 hidden md:flex lg:flex"
              alt="Hero Bg" loading="lazy"></NuxtImg>
          </div>
          <UICustomCard>
            <template #header>
            ...
            <template #body>
              ...
              <div class="flex flex-col gap-3 mt-5">
                <UserDashboardExamCardList v-for="item in summary" :key="item.exam_id" :title="item.exam_title"
                  :correct-answer="item.correctAnswer" :total-answer="item.totalQuestion"
                  :success-rate="item.success_rate" :color="item.color" />
              </div>
            </template>
          </UICustomCard>
          <UICustomCard>
            ...
            <template #body>
              <div class="flex flex-col gap-3 w-full">
                <UserDashboardCardList :has-trailing="true" v-for="(item, index) in courseData" :increment="index + 1"
                  :description="item.description">
                  <template #trailing-right><i-simple-icons:discourse
                      class="text-gray-500 dark:text-gray-100" /></template>
          ...
          <UICustomCard>
            ...
            <template #body>
              <div class="flex flex-col items-center justify-center gap-2">
                <CircleProgressBar stroke-width="5" size="200" color-filled="#062d19" :color-unfilled="hexColor"
                  animation-duration="1s" :value="percentage" :max="100" rounded>
                  <h2 class="font-semibold">{{ percentage.toFixed(2) }}%</h2>
                  <i-humbleicons:crown class="text-gray-500 font-bold" width="32" height="32" />
                </CircleProgressBar>
                <UBadge :color="colors as ProgressColor" class="py-1 mt-2 px-10 text-lg dark:text-white/90">{{ detail }}
                </UBadge>
    ...
    <template #fallback>
      <SkeletonUserDashboard></SkeletonUserDashboard>
    </template>
  </Suspense>
